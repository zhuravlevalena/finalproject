...
function Layout({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  return (
    <div className={`${isAuthPage ? 'min-h-screen' : 'h-screen'} bg-background flex flex-col font-body selection:bg-primary/30 selection:text-white overflow-hidden`}>
      {!isAuthPage && <Navbar />}
      <main className={`${isAuthPage ? 'flex-grow' : 'flex-1 overflow-y-auto'}`}>
        {children}
      </main>
    ...
    <Layout>
      <Switch>
        ...
        <Route path="/create-card">
          {isLogged ? <CreateCard /> : <Login />}
        </Route>
        <Route path="/login">
          {!isLogged ? <Login /> : null}
        </Route>
        <Route path="/register">
          {!isLogged ? <Register /> : null}
        </Route>
